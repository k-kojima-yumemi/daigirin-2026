...
Modの開発でよく使われるライブラリをフレームワークとこの記事では呼称しています。
一般的にはフレームワークと呼ぶよりもプラットフォームと呼ぶことの方が多いのですが、やっていること自体はWebのフレームワークと似ていると思っているのでこの記事ではそのように呼ぶことにしています。
...
ただ近年minorばかりが上がり続け、minorらしからぬmajorレベルの変更点が多数含まれるようになりました。
...
最近はMixinで対応することが増えたのでリフレクションを触る機会は減ったのですが、依然としてリフレクションは手軽なテクニックです。
...
同じJVMの別スレッドなのでstaticフィールドで状態を共有することはできてしまうのですが、マルチプレイで動かなくなるので御法度とされています。
一般的なWebサーバーと異なるのは、クライアントにもサーバーにも同じワールドがあることです。
...
クライアントとサーバーでは必要がある時、または定期的に情報を同期する必要があります。
Minecraftのロジックにすでに含まれているものはMod側で処理を書かなくても同期されます。
例えばワールドに設置されたブロックそのものは勝手に同期してくれます。
ブロックの種類や向きといった情報が同期されます。
しかし、一部を除くブロックの内部状態はデフォルトでは同期されません。

たとえばバニラのかまどと同じ機能を持つブロックを追加するとします。
この時かまどのインベントリについては内部状態のうち自動で同期される対象なのでクライアント側に同期されます。
しかし、明示的に同期の処理を書かないとかまどの動作の進捗状況は同期されません。
このとき、クライアント側では進捗は初期状態のままなので、例えば進捗0%のまま動いているようには見えず、しかしサーバー側で進捗100%になった瞬間完成品が出てくるように見えてしまいます。
これだとUXとしては適切ではないので、進捗状況をクライアント側に同期することでクライアント側で正しく描画されるようにします。
サーバーの情報をクライアントに伝えるためにパケットを送信します。
逆に、クライアントの操作(GUIのボタンを押したなど)を伝えるためにクライアントからサーバーにパケットを送信することもあります。

いずれにせよ、サーバーの状態が正となるようにしています。
クライアントでクライアントOnlyのModを導入し、クライアントのワールドを不正に書き換えて、その状態をサーバーに同期させる攻撃をされることがあります。
このような攻撃がまかり通ることのないように注意深く実装します。
Webでもサーバーはクライアントから送信されたデータを信用してはならず、バリデーションなどを徹底する必要があります。
それと同じです。
...
サーバーとクライアントで用途が分かれているため、サーバーに使われるJarとクライアント用のJarは異なります。
そして中に含まれるclassが一部異なっています。
サーバーでは画面がありません(厳密にはGUIのある環境で開始するとサーバー状態を表すGUIが開きますが)。
つまりサーバーには画面を表示する機能は不要なので、画面や描画に関するクラスは一切含まれていません。
マルチプレイをしている時に、サーバー側からクライアントにしか存在しないクラスやメソッドを呼び出してしまってクラッシュしてしまうというのはよくあるエラーの1つです。
特に最初のうちは手軽なのでシングルプレイでしか検証をしないため、このようなミスには一層気が付きにくいです。
存在していないクラスを継承しているといった場合に、そのクラスをロードしようとすると親クラスが存在しないためClassNotFoundExceptionのエラーになります。
ロードすることが条件なので、単にstaticメソッドやフィールドにアクセスするだけでエラーが発生し、クラッシュします。
...
最後に、Minecraftのバージョンアップへの追従について記載します。
Minecraftはリリース版が出る前にスナップショット、プレリリース、リリース候補が順に出ます。
Pre ReleaseやRelease Candidateが出たタイミングから、該当バージョン向けに公開されているフレームワークを使った移植作業をはじめていきます。
真っ先にFabricが対応されるので、Fabricでコンパイルでき、動作するように修正を加えていきます。
公式の出している変更点や修正点は半分程度参考になります。
ユーザー目線での差分がほとんどないため、裏側の処理が変わっていたとしても告知されることはありません。
それでも有用な情報が隠れていることがあります。
Data PackやResource Packのディレクトリ構造が変わっていることもあるので注意深くみます。
このようなバニラにも直接関係ある変更点は公式のChangelogに書かれているので大事です。

バニラの裏側の変更は唐突です。一般的なライブラリにあるようなDeprecatedや非推奨はありません。
いきなりロジックが整理されたり変更されたりします。
ひとまず落ち着いて、Fabric側でどのような変更をして追従しているかを調べたり、バニラのコードでどのような呼び出しになっているかをみていきます。
デコンパイルされたコードをひたすら辿っていく作業になります。
このような作業はAIに任せたいのが正直ですが、いい感じに依存関係のソースコードを読み取って分析してくれるような方法を見つけられていません。
LSPでできるはずなので、技術的には難しくないようには思うのですが。
...
クラスが見つからないと思ったら単にリネームされているだけということもあるので、IDEの一括置換が活躍します。

https://github.com/neoforged/.github/blob/main/primers/1.21.11/index.md

Fabricでコンパイルできたら動作確認を行い、Unit TestやGame Testが通るように修正を加えていきます。
そこまでできればFabricは対応完了です。

次に、別のフレームワークでの対応を行っていきます。
リリース版が出てからの対応になることが多いです。
Fabricで対応できていればあらかたバニラの変更点は把握できているため、Fabricで変更した箇所を同じように変更し、コンパイルが通るようにしていきます。
時たまフレームワーク側のAPI変更に追従する必要があります。
フレームワークでの変更はブログ記事が書かれていたり、少し前のバージョンからDeprecatedの宣言がされているのでMinecraft本体の変更に比べれば追従は容易です。

## おわりに

Minecraft moddingについて紹介してみました。
さわりだけではありますが、少しでも世界観に興味を持ってもらえれば幸いです。
Minecraftには興味があった、遊んでいるけど少し追加が欲しいと思っている人の助けになればと思います。

何ができるのか、どうやって実装するのかという話は多くはありませんが公開されています。
GitHubをAIに見て貰えばヒントも多数収集できるでしょう。
